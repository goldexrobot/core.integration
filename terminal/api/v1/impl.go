...
type Hardwarer interface {
	...
	SpectralEval() (eval ImplSpectralData, netFail, rejectedEval bool, err error)
	HydroEval() (eval ImplHydroData, netFail, rejectedEval, unstableScale bool, err error)
	FinalizeEval() (fineness ImplFinenessData, netFail, rejectedEval bool, err error)
...
func (a *Impl) EvalSpectrum() (res EvalSpectrumResult, err error) {
	...
	eval, netFail, rejectedEval, err := a.hw.SpectralEval()
	if err != nil {
		err = a.brokenProxy(err)
		return
	}
...
func (a *Impl) EvalHydro() (res EvalHydroResult, err error) {
	...
	eval, netFail, rejectedEval, unstableScale, err := a.hw.HydroEval()
	if err != nil {
		err = a.brokenProxy(err)
		return
	}
	...
	switch {
	case netFail, rejectedEval, unstableScale:
	...
	default:
		...
		fineness, netFail, rejectedEval, xerr := a.hw.FinalizeEval()
		if xerr != nil {
